...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    public HashMap(int initialCapacity, float loadFactor) {
        ...
        //指定capacity时，生成的tableSize为大于capacity的最小2^n
    ...
    final Node<K,V> getNode(int hash, Object key) {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            (first = tab[(n - 1) & hash]) != null) { //根据hash值算出node所在的位置下标
            if (first.hash == hash && // 从第一个元素开始检查
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)//tab[i]处没有元素 ，n-1后n的二进制数的最后一位一定是1
            ...
            if (p.hash == hash &&
                ...
                //若key 是值类型 用== 比较， 若是引用类型需要equals方法比较key的内容是否相同
                // tab[i]处的key与传入的key相同
                e = p;
            else if (p instanceof TreeNode)//p是红黑树时
                e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value);
            else {
                for (int binCount = 0; ; ++binCount) {//记录 tab[i]处的元素个数
                    if ((e = p.next) == null) {//遍历链表，并在尾部插入新节点
                        p.next = newNode(hash, key, value, null);
                        if (binCount >= TREEIFY_THRESHOLD - 1) // -1 for 1st
                            treeifyBin(tab, hash);//链表长度>=8时， 判断capacity是否大于64，若大于则进行树化并退出循环
                        break;
                    }
                    if (e.hash == hash &&
                        ((k = e.key) == key || (key != null && key.equals(k)))) //在链表中已经存在对应的key
                        break;
                    p = e;
                }
            }
            if (e != null) { // existing mapping for key key已经存在时，返回oldvalue
                V oldValue = e.value;
        ...
        ++modCount;//记录修改次数，供iterator抛出fast fail
        if (++size > threshold)
            resize();//扩容
    ...
    final Node<K,V>[] resize() {
        ...
        int newCap, newThr = 0;
        if (oldCap > 0) {
            if (oldCap >= MAXIMUM_CAPACITY) { //oldCap大于最大容量时，不进行扩容
                threshold = Integer.MAX_VALUE;
                return oldTab;
            }
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                     oldCap >= DEFAULT_INITIAL_CAPACITY) //2倍扩容后的容量小于最大容量
                newThr = oldThr << 1; // double threshold
        }
        //tab未初始化时
        else if (oldThr > 0) // 构造方法中指定了capacity
            newCap = oldThr;
        else {               // zero initial threshold signifies using defaults
            newCap = DEFAULT_INITIAL_CAPACITY;
            newThr = (int)(DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY);
        }
        if (newThr == 0) { //当扩容后的容量大于最大容量时，与构造方法中指定了capacity时
        ...
        if (oldTab != null) {
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null;
                    if (e.next == null) //当tab[j]处只有一个元素时
                        ...
                        do {
                            next = e.next;
                            if ((e.hash & oldCap) == 0) { //将tab[j]处的链表进行分组；
                                if (loTail == null)
                                    loHead = e;
                                else
                                    loTail.next = e;
     ...
     */
    final void treeifyBin(Node<K,V>[] tab, int hash) { //多线程情况下，树化会出现死循环
        int n, index; Node<K,V> e;
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY)
            resize(); //tab.lenght < 64 时进行resize();
    ...
    final int capacity() {
        return (table != null) ? table.length :
            ...
            DEFAULT_INITIAL_CAPACITY;
        //等价于 if(table != null) return table.length;
        //      else if(threshold > 0) return threshold;
        //      else return DEFAULT_INITIAL_CAPACITY;
    }
