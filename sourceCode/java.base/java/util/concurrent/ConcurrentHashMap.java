...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    /* ---------------- Fields -------------- */
    //所有字段都被volatile修饰，保证内存可见性，使用get()时不需要加锁
    /**
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        if (key == null || value == null) throw new NullPointerException(); //key,value不能为null
        int hash = spread(key.hashCode()); //离散hash值
        int binCount = 0;
                ...
                synchronized (f) { //在tab[i]处是链表时，锁的是第一个节点，红黑树时锁的是整个TreeBin对象
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) { //tab[i]还是链表
                            ...
                            Node<K,V> p;
                            binCount = 2; //已经是红黑树，就不需要binCount来判断是否需要树化
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            if ((sc = sizeCtl) < 0) //sizeCtl < 0 表示正在扩容或正在初始化
                Thread.yield(); // lost initialization race; just spin
            else if (U.compareAndSetInt(this, SIZECTL, sc, -1)) { //cas的设置sizeCtl为-1
                try {
                    if ((tab = table) == null || tab.length == 0) {
                        int n = (sc > 0) ? sc : DEFAULT_CAPACITY; //判断有没有指定capacity
                        @SuppressWarnings("unchecked")
